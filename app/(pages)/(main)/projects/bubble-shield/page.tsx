  ...
  return (
    <div>
      ...
      <p>
        ...
        , and later included as an ability in the game. As a lover of sci-fi the
        shot below from the trailer particularly stood out to me, as it seems to
        have done for a lot of people given the number of "bubble shield"
        references across the internet. The appearance of the shield changed
        fairly significantly when it was implemented in-game, and that's what
        I've used as my primary source of inspiration.
      </p>
      ...
      <p>
        ...
        wanted to build the sphere mesh in Houdini so that I could easily tweak
        parameters such as the resulting hexagon size, and I also wanted to
        modify how the effect materialised on screen but more on that later.
      </p>
      <h3>Sphere Mesh</h3>
      <p>
        The initial sphere was built in Blender using the Geodesic Dome add-on.
        The add-on gets you most of the way there with a hexagonal sphere mesh,
        but for managing textures such as for the refraction it'd be useful to
        have correctly UV mapped triangles as our base polygon. With triangles
        we can use a simple square texture with a line across the bottom which
      ...
      <p>
        ...
        <code>Poke faces</code> tool, and then using the UV tools to stack every
        triangle on top of each other in the UV map (the{' '}
        <ExternalLink href="https://github.com/SavMartin/TexTools-Blender">
          TexTools
        </ExternalLink>{' '}
        add-on was a great help here). However if we want to change the original
        sphere's shape, to tweak the size of the hexagons making up the sphere
        for example, then we need to manually repeat these steps. Enter
        Houdini's procedural workflow!
      </p>
      <p>
        Using Houdini we can instead build up the hexagonal sphere and UV map it
        using SOP nodes. You can see the node graph below, but the general idea
        is to:
      </p>
      <ol className="list-decimal marker:text-pink-400 ml-8 mr-2">
        ...
        <li>
          As Houdini has no equivalent to <code>Poke faces</code> in Blender, we
          need to then inset every face, scale the inset down to 0, then fuse
          the new vertices at the centre of each hexagon together in order to
          get our triangles.
        </li>
        <li>
        ...
        <li>
          Use the UV Unitize SOP from SideFX Labs to stack every face on top of
          each other, all stretched to maximally fit the UV map.
        </li>
      ...
      <p>
        The textures we need for the sphere are a refraction and normal map. You
        could do this in Photoshop or similar but again I wanted flexibility in
        order to try out some different effects, so I went for a procedural
        approach using Substance Designer. The general idea here is to have a
        slightly blurry black line denoting the strongest area of refraction,
        which will roughly form the shape of hexagons with rounded edges.
      </p>
      <figure>
      ...
      <p>
        ...
        we'll use the Fresnel component to drive these properties. It will also
        be helpful to factor in a depth fade so that we don't end up with some
        unsightly seams between our bubble shield and the ground (or any other
        opaque object).
      </p>
      <figure>
      ...
      <p>
        ...
        effect however, is that it just sort of <em>poofs</em> into existence. I
        think it'd be much cooler if the effect built up (or down) vertically,
        as if the shield was "spreading out" from an initial starting point.
      </p>
      <p>
        To achieve this effect I implemented a dissolve animation, tweaked to
        use the object's local up axis. There's a lot of artistic freedom here,
        but I think the effect looks best if we use a very low-scale noise as
        demonstrated in the final effect below. The output from the dissolve
        calculation needs to be factored into the emission, opacity, normal, and
        refraction which leads to a slightly messy graph unfortauntely, but I've
      ...
      <p>
        ...
        can be significantly tweaked in-engine to allow for numerous variations
        in-game. Hope you like it!
      </p>
      <p>
